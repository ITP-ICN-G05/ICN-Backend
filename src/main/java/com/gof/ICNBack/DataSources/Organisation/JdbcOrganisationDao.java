package com.gof.ICNBack.DataSources.Organisation;

import com.gof.ICNBack.Entity.Organisation;

import java.util.List;
import java.util.Map;

public class JdbcOrganisationDao extends OrganisationDao {


    @Override
    public List<Organisation.OrganisationCard> searchOrganisationCards(double locationX, double locationY, double endX, double endY, Map<String, String> filterParameters, String searchString, Integer skip, Integer limit) {
        return null;
    }

    @Override
    public Organisation getOrganisationById(String organisationId) {
        return null;
    }

    @Override
    public List<Organisation.OrganisationCard> getOrgCardsByIds(List<String> orgIds) {
        return null;
    }

    @Override
    public List<Organisation> getOrganisationsWithoutGeocode() {
        return null;
    }

    @Override
    public void updateGeocode(List<Organisation> orgs) {

    }
}
